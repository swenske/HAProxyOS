...
func TestDmVerityDataDevice(t *testing.T) {
	cases := []struct {
		...
		{
			name: "single-disk GPT layout (image/disk/assemble.sh, hack/qemu-ab-boot-test.sh slot A)",
			// The exact string a real boot produced (see rootfs/init's
			// own development notes) - not a hand-simplified stand-in.
			cmdline: `console=ttyS0 panic=-1 dm-mod.create="vroot,,,ro,0 15608 verity 1 /dev/vda1 /dev/vda2 4096 4096 1951 1 sha256 58b57af748b7f4ff334c9f9612c2c62a7dd28d3753c70ed03cd409fa7b7d30e4 eee8298dddb05a935d91fc88bdca004d1e3baec7dccd0a03da9a552d4142212f" root=/dev/dm-0 rootfstype=squashfs ro`,
			want:    "/dev/vda1",
			wantOK:  true,
		},
		{
			name:    "single-disk GPT layout, slot B",
			cmdline: `console=ttyS0 dm-mod.create="vroot,,,ro,0 15608 verity 1 /dev/vda3 /dev/vda4 4096 4096 1951 1 sha256 abc def" root=/dev/dm-0`,
			want:    "/dev/vda3",
			wantOK:  true,
		},
		{
...
func TestStatePartitionDevice(t *testing.T) {
	cases := []struct {
		...
		{name: "slot A data partition", dataDev: "/dev/vda1", want: "/dev/vda5", wantOK: true},
		{name: "slot B data partition", dataDev: "/dev/vda3", want: "/dev/vda5", wantOK: true},
		{name: "double-digit partition number", dataDev: "/dev/vda12", want: "/dev/vda5", wantOK: true},
