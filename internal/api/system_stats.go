...
func (s *System) Memory(_ context.Context, _ *emptypb.Empty) (*haproxyosv1alpha1.MemoryResponse, error) {
	...
	resp := &haproxyosv1alpha1.MemoryResponse{}
...
func (s *System) CPUInfo(_ context.Context, _ *emptypb.Empty) (*haproxyosv1alpha1.CPUInfoResponse, error) {
	...
	resp := &haproxyosv1alpha1.CPUInfoResponse{}
	var cur *haproxyosv1alpha1.CPUInfo
	sc := bufio.NewScanner(f)
	for sc.Scan() {
		line := sc.Text()
		if line == "" {
		...
		if key == "processor" {
			...
			cur = &haproxyosv1alpha1.CPUInfo{Processor: uint32(n)}
...
func (s *System) LoadAvg(_ context.Context, _ *emptypb.Empty) (*haproxyosv1alpha1.LoadAvgResponse, error) {
	...
	return &haproxyosv1alpha1.LoadAvgResponse{Load1: load1, Load5: load5, Load15: load15}, nil
}
...
func (s *System) DiskStats(_ context.Context, _ *emptypb.Empty) (*haproxyosv1alpha1.DiskStatsResponse, error) {
	...
	resp := &haproxyosv1alpha1.DiskStatsResponse{}
	sc := bufio.NewScanner(f)
	for sc.Scan() {
		...
		resp.Disks = append(resp.Disks, &haproxyosv1alpha1.DiskStat{
			DeviceName:     name,
			ReadCompleted:  reads,
